...
if __name__ == "__main__":
    ...
    f = open("./output/res_execute_mutant.txt", 'r')
    list_execute_mutant = f.read().split('\n')[0:-1]
    f.close()

    f = open("./output/res_record.txt", 'r')
    list_mutant_record = f.read().split('\n')[0:-1]
    f.close()

    with open("./output/all_execute_mutant.txt", 'w') as f :
        f.write("")   # create and clean all_execute_mutant.txt
        f.close()

    f = open("./output/all_execute_mutant.txt", 'a')
