...
if __name__ == "__main__":
    ...
    list_can_mutate_line_index = []
    for str_record in list_res_record:
        source_line_txt = int(str_record.split("line: ")[1].split("   index:")[0])
        if source_line_txt not in list_can_mutate_line_index:
            list_can_mutate_line_index.append(source_line_txt)

    with open("./output/suspicious_SBFL.txt", 'r') as f:
    ...
    for source_line_txt in list_source_line_txt:
        ...
        if source_line in list_can_mutate_line_index:
    ...
    with open("./_SBFL_randomization_higher_order_mutant_set/res_mutant_line.txt", 'w') as f:
        f.write("")
    f.close()

    min_order_num = parserCommad().min_order_num
    max_order_num = parserCommad().max_order_num
    f = open("./_SBFL_randomization_higher_order_mutant_set/res_mutant_line.txt", 'a')
