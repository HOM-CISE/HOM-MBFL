...
f_result = open("./EXAM_rank_result.csv", 'w')
...
def calcRank(list_fault_line, suspicious_file_root):
    suspicious_file_path = suspicious_file_root + "suspicious_SBFL.txt"
    ...
    for Jaccard_suspicious in list_Jaccard_suspicious:
        ...
        dict_sus_Jaccard[mutant_line] = sus_mutant_line
    ...
    for Ochiai_suspicious in list_Ochiai_suspicious:
        ...
        dict_sus_Ochiai[mutant_line] = sus_mutant_line
    ...
    for Op2_suspicious in list_Op2_suspicious:
        ...
        dict_sus_Op2[mutant_line] = sus_mutant_line
    ...
    for Tarantula_suspicious in list_Tarantula_suspicious:
        ...
        dict_sus_Tarantula[mutant_line] = sus_mutant_line
    ...
    for Dstar3_suspicious in list_Dstar3_suspicious:
        ...
        dict_sus_Dstar3[mutant_line] = sus_mutant_line
    ...
    # with open(suspicious_file_root + "suspicious_first_order_rank.txt", 'w') as f:
...
def calcEXAMScore(list_fault_line, suspicious_file_root):
    # 1. return rank of all types of suspicious, and output "suspicious_rank.txt" in suspicious_file_root.
    tuple_rank_result = calcRank(list_fault_line, suspicious_file_root)
    ...
    # num_fault_line = 5   # =========================(configure fault line rank)

    # f_result.write(suspicious_file_root+"\n\n\n")
    ...
    for num_fault_line in range(1, len(list_fault_line)+1):
        ...
        f_result.write(str(Jaccard_EXAM) + ',' + str(Jaccard_rank) + ',')
        ...
        f_result.write(str(Ochiai_EXAM) + ',' + str(Ochiai_rank) + ',')
        ...
        f_result.write(str(Op2_EXAM) + ',' + str(Op2_rank) + ',')
        ...
        f_result.write(str(Tarantula_EXAM) + ',' + str(Tarantula_rank) + ',')
        ...
        f_result.write(str(Dstar3_EXAM) + ',' + str(Dstar3_rank) + ',')
...
if __name__ == "__main__":
    ...
    for version_i in range(1, 1762+1):
    # for version_i in range(1545, 1752+1):
    # for version_i in range(1753, 1762 + 1):
        try:
            suspicious_file_root = "./v" + str(version_i) + "/output/"
            with open("./v" + str(version_i) + "/output/Fault_Record.txt") as f:
                str_fault_record = f.read()
            f.close()
            list_fault_line = []
            list_fault_line_temp = str_fault_record.split("Line ")[1:]
            for fault_line_temp in list_fault_line_temp:
                list_fault_line.append(fault_line_temp.split(": ")[0].strip())
            print(list_fault_line)
            calcEXAMScore(list_fault_line, suspicious_file_root)
        except FileNotFoundError:
            f_result.write("-,-,-,-,-,-,-,-,-,-,-,-,-,-,-,-,-,-,-,-,-,-,-,-,-,-,-,-,-,-,-,-\n")
            print("v" + str(version_i) + " is not exists!\n")
