...
def Mutation(filename1, filename2, root1, root2):
    ...
    for data in first_data:
        ...
        for res_record_line in list_res_record_line:
            ...
            mu_source[original_line - 1] = mutated + '\n'
        f.close()

        f = open(dirTo, 'w')
        for mu_source_line in mu_source:
            f.write(mu_source_line)
        f.close()
...
if __name__ == "__main__":
    ...
    str_mutant_source_path = "./_RQ1_mutant_set/mutant_set_ideal/Mutant_source/"
    if not os.path.exists(str_mutant_source_path):
        os.mkdir(str_mutant_source_path)

    Mutation("./_RQ1_mutant_set/mutant_set_ideal/res_record.txt", defect_source_name,
