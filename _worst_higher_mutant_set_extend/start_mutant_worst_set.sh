worst_MBFL_run_starttime=`date +'%Y-%m-%d %H:%M:%S'`


echo "==========[worst_MBFL_run_starttime]: "worst_MBFL_run_starttime
echo "=================worst_MBFL_run_initialize args:"
...
rm -rf ./_worst_mutant_set
echo "======================(accept)clear_temp_file==========================="
mkdir ./_worst_mutant_set


# 创建可变异的变异行行数列表
python3 ./worst_higher_mutant_set_extend/create_res_mutant_line.py
echo "==================== (accept)create worst_mutant_set & res_mutant_line.txt ======================"


# 随机选择两个变异体行，组合成二阶变异体列表
python3 ./worst_higher_mutant_set_extend/create_res_record.py
...
cp -r $defect_source_dir ./_worst_mutant_set/


#=========================================================================================================
# 创建变异体源代码（将被测程序拷贝后重命名，修改文件中的内容）
python3 ./worst_higher_mutant_set_extend/create_mutants.py --mutant_root ./_worst_mutant_set/ --defect_source_path $defect_source_path
echo "==================== (accept)combine mutant sources of each set ======================"


# 创建编译变异体程序脚本，并执行
python3 ./worst_higher_mutant_set_extend/create_compile_mutants_shell.py --mutant_root ./_worst_mutant_set/
...
# 记录可运行的变异体（通过编译的变异体）
python3 ./worst_higher_mutant_set_extend/create_res_execute_mutant.py --mutant_root ./_worst_mutant_set/
echo "==================== (accept)create res_execute_mutant.txt ==============================="

# 统计所有可运行变异体的详细信息，即按照res_execute_mutant.txt文件的变异体序号组合res_record.txt文件的变异体详细信息
python3 ./worst_higher_mutant_set_extend/create_all_execute_mutants.py --mutant_root ./_worst_mutant_set/
echo "==================== (accept)create all_execute_mutant.txt ==============================="


# 生成变异体执行脚本create_res_original_fault_version_in.sh
python3 ./worst_higher_mutant_set_extend/create_res_original_fault_version_in_shell.py --inputs_dir $inputs_dir --inputs_type $inputs_type --true_root_dir $true_root_dir --defect_root_dir $defect_root_dir --mutant_root ./_worst_mutant_set/ --timeout $timeout
...
python3 ./worst_higher_mutant_set_extend/clac_worst_sus.py
python3 ./worst_higher_mutant_set_extend/combine_worst_sus.py
python3 ./worst_higher_mutant_set_extend/clac_worst_EXAM_score.py
...
echo ""
worst_MBFL_run_endtime=`date +'%Y-%m-%d %H:%M:%S'`
echo "=====[starttime]: "$worst_MBFL_run_starttime
echo "=====[endtime]: "$worst_MBFL_run_endtime
start_second=$(date --date="$worst_MBFL_run_starttime" +%s)
end_second=$(date --date="$worst_MBFL_run_endtime" +%s)
echo "run time(sec): "$((end_second-start_second))
echo "=====[starttime]: "$worst_MBFL_run_starttime >> ./time.log
echo "=====[endtime]: "$worst_MBFL_run_endtime >> ./time.log
