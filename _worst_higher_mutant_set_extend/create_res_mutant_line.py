...
def _MutantLine(wait_fault_num, list_nonfault_line, list_mutant_line):
    if wait_fault_num == 0:
    ...
    for non_fault_line in list_nonfault_line:
        ...
        _MutantLine(wait_fault_num-1, list_nonfault_line, list_mutant_line_copy)


def recordMutantLine(list_file_res_mutant_line, fault_num, list_nonfault_line):
    list_file_res_mutant_line.append([])
    _MutantLine(fault_num, list_nonfault_line, [])
    global file_res_mutant_line
...
if __name__ == "__main__":
    ...
    fault_num = 2
    recordMutantLine(list_file_res_mutant_line, fault_num, list_nonfault_line)

    # for file_res_mutant_line in list_file_res_mutant_line:
    #     print(file_res_mutant_line)

    with open("./_worst_mutant_set/res_mutant_line.txt", 'w') as f:
