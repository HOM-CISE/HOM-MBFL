...
{ 
  ...
  int id=0;
  char ch,ch1[2];
  for (j=0;j<=80;j+=2)          /* initial the buffer   */
      { buffer[j]='\0';} 
   ch1[0]='\0';
   ch1[1]='\0';
   ch=get_char(tp);
   ...
   if(is_spec_symbol(ch1)==FALSE)     /* if end character is special_symbol */
...
{ char ch1[2];  /* fixed array declaration MONI */
 ...
 if(str_com_id==0)          /* is string token */
